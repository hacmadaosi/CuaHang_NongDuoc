...
namespace CuahangNongduoc
{
    ...
    public class ThamSo
    {
        ...
        public static long TaiKhoan
        {
            get
            {
                DataService ds = new DataService();
                object obj = ds.ExecuteScalar(new OleDbCommand("SELECT TAI_KHOAN FROM THAM_SO"));
                if (obj == null || obj == DBNull.Value)
                    return 1; // Giá trị mặc định nếu chưa có
                return Convert.ToInt64(obj);
            }
            set
            {
                DataService ds = new DataService();
                ds.ExecuteNoneQuery(new OleDbCommand("UPDATE THAM_SO SET TAI_KHOAN = " + value));
            }
        }
